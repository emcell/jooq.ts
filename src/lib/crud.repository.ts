...
import { DSL, DbTypes } from './dsl/dsl';
...
export class CrudRepository<T, PK extends keyof T> {
  ...
  get primaryKeyField(): Field<T[PK], DbTypes> {
    return this.tableDefinition.fields[this.primaryKey] as Field<
      T[PK],
      DbTypes
    >;
  }
  ...
  private removeUnmatchingFieldsAndCopyIfNecessary<ObjectType extends object>(
    object: ObjectType,
  ...
  public async insertAutoGenerated(object: Except<T, PK>): Promise<T> {
    ...
    const o = this.create
      .insertInto<Except<T, PK>>(this.tableWithoutPK, [object])
  ...
  public async insertAllAutoGenerated(objects: Except<T, PK>[]): Promise<T[]> {
    ...
    return this.create
      .insertInto<Except<T, PK>>(this.tableWithoutPK, convertedObjects)
      .returning(this.tableDefinition.fields)
  ...
  protected hasAnyUsableFields(object: Subset<T>): boolean {
    const o: any = object;
    const fields: any = this.tableDefinition.fields;
    for (const k in o) {
      if (fields[k]) {
        return true;
      }
    ...
    // {
    //   fieldname: undefined // setting this field to null in databse
    // }
    // would not be found becaus o['fieldname'] would still be falsly
    // the other way round i find the key 'fieldname' in the object and also in the fields of this instance
  ...
  public async update(id: T[PK], object: Subset<T>): Promise<T> {
  ...
  public async insertOrUpdate(object: SetOptional<T, PK>): Promise<T> {
    ...
    if (pk === undefined) {
      return this.insertAutoGenerated(object as unknown as Except<T, PK>);
    } else {
      return this.update(pk, object as unknown as Subset<T>);
    }
  }
}
